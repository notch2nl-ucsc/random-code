...
import sys
# bring in my library
sys.path.insert(0, '/mnt/ifiddes/Comparative-Annotation-Toolkit/CAT')
import pysam
import argparse
...
def parse_args():
    ...
    parser.add_argument('--consensus_ref', default='/mnt/ifiddes/notch2nl_datafiles/consensus_bwa_index/consensus.fa')
...
def find_notch_barcodes(inbam, regions):
    ...
    for region in regions:
        ...
        for rec in aln_iter:
            if rec.is_duplicate or rec.is_secondary or rec.is_supplementary or rec.is_read2:
                continue
            try:
                tag = rec.get_tag('BX')
                barcodes.add(tag)
...
def write_recs_to_fastq(rec_dict, tmp_fastq):
...
def align_fastq(tmp_fastq, bwa_index, tmp_bam):
    """run the BWA alignment"""
    tmp = get_tmp_file()
    bwa_cmd = [['bwa', 'mem', '-t', 16, bwa_index, tmp_fastq, '-R', '@RG \tID:NOTCH2NL\tSM:NOTCH2NL'],
               ['samtools', 'view', '-b', '-'],
               ['sambamba', 'sort', '-t', 8, '-m', '15G', '-o', '/dev/stdout', '/dev/stdin']]
    run_proc(bwa_cmd, stdout=tmp_bam)
...
def main():
    ...
    tag_map = write_recs_to_fastq(notch_recs, tmp_fastq)
    align_fastq(tmp_fastq, args.consensus_ref, tmp_bam)
